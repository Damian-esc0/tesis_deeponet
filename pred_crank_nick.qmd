...
```{julia}
#| lst-label: lst-met-num
#| lst-cap: Método de Crank-Nicolson.
#| output: false
...
```{julia}
#| lst-label: lst-met-num
#| lst-cap: Método de Crank-Nicolson (Parte 1).
#| output: false
using DifferentialEquations, LinearAlgebra
using DataFrames, CSV
...
```
```{julia}
#| lst-cap: Método de Crank-Nicolson (Parte 2).
#| output: false
# --- SISTEMA DE EDOs DEL PDE -------------------------------------------
function f!(du, u, _, τ)
    U = reshape(u, Nx, Ny)          # arreglo 2D con los valores actuales
...
```{julia}
#| lst-cap: Método de Crank-Nicolson (Parte 3).
#| output: false
# --- RESOLVER PDE -----------------------------------------------------
τspan = (0.0, 1.0)
prob = ODEProblem(f!, u0, τspan)
...
#| fig-cap: "Resultados obtenidos mediante el método numérico de Crank Nickolson, para la temperatura T en función de las coordenadas X e Y, aquí el código de colores representa el gradiente de temperaturas; todas las medidas están adimensionalizadas dadas las ecuaciones [-@eq-adimencionalizadores]."
...
Este archivo permitió calcular de manera estandarizada las métricas de error (MAE, MaxAE, error L2) en la sección de comparación de resultados [@sec-val-cuant-Cr-Nc].
:::