...
Los errores obtenidos demuestran la eficacia del modelo para converger a la condición inicial, pues tal como se aprecia en la @fig-abs-errors, a medida que la ecuación evoluciona en el tiempo, las predicciones entre el método de Crank Nickolson y la red neuronal divergen, esto es conforme evoluciona la función, vemos que cada vez se aleja más del valor real. Valdría la pena en otro trabajo comparar la solución con otro método númerico clásico para observar si se presenta el mismo comportamiento.Desafortunadamente no se cuenta con una base de datos que sirva como conjunto de validación por lo que no se puede asegurar que los resultados del método numérico sean los más óptimos para tomar como referencia.
