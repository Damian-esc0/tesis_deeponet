...
Esta sección presenta un análisis cualitativo de los resultados mediante la comparación directa entre las predicciones del modelo, las soluciones reportadas en el estudio de @medical_rep, así como las obtenidas mediante el método de Crank Nickolson y la solución analítica. La visualización paralela permite evaluar:
...
El siguiente código grafica las predicciones de la red neuronal DeepONet para la temperatura dadas las coordenadas espaciales en los distintos tiempos de interés, los ejes están adimencionalizados siguiendo las ecuaciones -@eq-adimencionalizadores.
```{python}
...
for i, (t_val, ax) in enumerate(zip(times, axes)):
    ...
    ax.set_zlabel("T [θ]", labelpad=10, rotation=90)
...
cbar.set_label('Temperatura [θ]')

plt.show()
```


![Resultados reportados por @medical_rep en el caso 2D.](images/results_paper.png){#fig-results_fnn fig-align="center" width="650"}

La comparación visual entre las predicciones del modelo DeepONet [@fig-my_results] y los resultados de @medical_rep [@fig-results_fnn] revela una notable similitud en la evolución temporal y espacial de la temperatura. Ambos modelos capturan la misma tendencia de calentamiento progresivo, con un gradiente térmico que se intensifica cerca de la frontera derecha ($x=1$), donde se aplica una condición de Neumann no homogénea. Sin embargo, se observa que en $t=0$, el modelo DeepONet se aproxima con mayor fidelidad al plano $XY=0$, lo que sugiere una mejor captura de la condición inicial en comparación con el trabajo de referencia.

### Modelo contra método numérico
La siguiente gráfica utiliza los archivos csv obtenidos de los códigos [-@lst-modelo-don] y [-@lst-met-num] para motrar una comparativa entre los tiempos de interés, las predicciones del modelo se encuentran en la parte superior mientras que las del método numérico en la parte inferior.
```{python}
#| label: fig-met-num-vs-model
#| fig-cap: "Contraste de las predicciones entre el modelo y el método de Crank Nicolson para cada tiempo. Se aprecia que ambas comparten forma y tendencia, sin embargo a medida que el tiempo se acerca a t=1 los resultados divergen."
...
cbar.set_label('Temperatura [θ]', labelpad=10)


plt.show()
```

Al contrastar las predicciones del modelo con las obtenidas mediante el método de Crank-Nicolson [@fig-met-num-vs-model], se confirma que ambas soluciones comparten la misma estructura general y comportamiento temporal. No obstante, a medida que el tiempo avanza hacia $t=1$, se aprecia una ligera divergencia en la magnitud de la temperatura, especialmente en la región cercana a $x=1$, donde el gradiente impuesto introduce mayor sensibilidad numérica.

### Modelo contra solución analítica
La siguiente gráfica utiliza los archivos csv obtenidos de los códigos [-@lst-modelo-don] y [-@lst-sol-ana] para mostrar una comparativa entre los tiempos de interés, las predicciones del modelo se encuentran en la parte superior mientras que las de la solución analítica en la parte inferior.
```{python}
#| label: fig-sol-ana-vs-model
#| fig-cap: "Contraste de las predicciones entre el modelo y la solución analítica para cada tiempo. Se aprecia que ambas comparten forma y tendencia, sin embargo a medida que el tiempo se acerca a t=1 los resultados divergen."
...
```{python}
#| label: tbl-errores-num
#| tbl-cap: "Desviación del modelo DeepONet respecto a Crank-Nicolson."
#| echo: false
...
Markdown(error_table)
```

Las métricas de error calculadas —Error Absoluto Medio (MAE), Error Absoluto Máximo (MaxAE) y Error L2— confirman el buen desempeño del modelo DeepONet. En la comparación con el método de Crank-Nicolson [@tbl-errores-num], el MAE se mantuvo entre 0.013 y 0.069, con un valor máximo de 0.181 en el MaxAE. Estos valores reflejan una aproximación satisfactoria, aunque se observa que los errores tienden a aumentar en tiempos intermedios ($t=0.25$ y $t=0.5$), posiblemente debido a la mayor complejidad dinámica en esas etapas.

#### Gráficas de error absoluto
```{python}
#| label: fig-abs-errors-num
#| fig-cap: "Errores absolutos entre el modelo y el método de Crank Nicolson para cada tiempo."
...
### Modelo contra la solución analítica
De manera análoga a la sección anterior se realizó una comparativa contra la solución analítica (@sec-sol-ana). De este modo se tiene una visión más completa acerca del rendimiento del modelo.
...
```{python}
#| label: tbl-errores-ana
#| tbl-cap: "Error del modelo DeepONet respecto a la solución analítica."
#| echo: false
from tabulate import tabulate
from IPython.display import Markdown
...
Al comparar con la solución analítica [@tbl-errores-ana], los errores son consistentemente bajos, con un MAE máximo de 0.067 y un MaxAE de 0.178. La similitud entre ambas tablas sugiere que el método de Crank-Nicolson y la solución analítica están bien alineados, y que el modelo DeepONet se aproxima a ambos con un nivel de error comparable.

#### Gráficas de error absoluto
```{python}
#| label: fig-abs-errors-ana
#| fig-cap: "Errores absolutos entre el modelo y la solución analítica para cada tiempo."
...
Los mapas de error absoluto [@fig-abs-errors-num] y [@fig-abs-errors-ana] permiten localizar espacialmente las discrepancias. Se observa que los mayores errores se concentran en la región de $x=1$, donde la condición de Neumann no homogénea introduce mayores exigencias en la aproximación. Esta distribución del error es coherente con el comportamiento reportado en la literatura para problemas con condiciones de frontera variables en el tiempo.
:::